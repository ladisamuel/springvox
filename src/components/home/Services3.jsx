...
function ServiceCard({ service }) {
  ...
  return (
    ...
    >
      ...
      <span className="pointer-events-none absolute right-0 top-0 h-10 w-10 rounded-tr-2xl
...
function useCarouselRow(direction, n, globalPause) {
  ...
  const tick = useCallback((now) => {
    ...
    if (s.autoRunning && !s.isDragging && !paused) {
      ...
      s.lastTime = now
      s.x += direction === 'rtl' ? -(AUTO * dt / 16.67) : (AUTO * dt / 16.67)
      clampLoop()
      applyTransform()
    } else {
      s.lastTime = now
  ...
  useEffect(() => {
    ...
    const pauseSelf  = () => { s.isHovered = true; s.autoRunning = false }
    const resumeSelf = () => {
      s.isHovered = false
  ...
  }, [tick, onDown, onMove, onUp, onWheel, applyTransform, updateCardTransforms, globalPause])
...
function CarouselBand({ services, direction, globalPause }) {
  ...
  return (
    <div style={{ perspective: 1400, perspectiveOrigin: 'center center' }}>
      <div
        ref={outerRef}
      ...
      >
        <div
          ref={trackRef}
          className="flex"
          style={{ gap: GAP, willChange: 'transform', transformStyle: 'preserve-3d' }}
        >
    ...
    <section id="services" ref={sectionRef} className="relative py-24 lg:py-32 overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <motion.div
          ...
          className="text-center mb-14 section-padding"
        >
        ...
        <motion.div
          ...
          className="mb-4"
        >
          <CarouselBand services={ROW_1} direction="rtl" globalPause={globalPause} />
        </motion.div>
        ...
        >
          <CarouselBand services={ROW_2} direction="ltr" globalPause={globalPause} />
        </motion.div>
